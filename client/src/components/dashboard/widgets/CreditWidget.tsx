import { useQuery } from "@tanstack/react-query";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { ChevronLeft, ChevronRight, Clock, CreditCard, AlertCircle, CheckCircle, XCircle } from "lucide-react";
import { format, parseISO, isToday, isAfter, isBefore, addDays } from "date-fns";
import { fr } from "date-fns/locale";
import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { api } from "@/lib/api";

interface Credit {
  id: number;
  amount: number;
  description: string;
  date: string;
  status: string;
  type: string;
  property?: {
    name: string;
  };
}

export function CreditWidget() {
  const [currentCreditIndex, setCurrentCreditIndex] = useState(0);
  const [creditToUpdate, setCreditToUpdate] = useState<{ id: number; status: string } | null>(null);
  const [creditFilterMode, setCreditFilterMode] = useState<'today' | 'week' | 'all'>('week');

  // Récupérer les crédits
  const { data: credits = [], isLoading } = useQuery<Credit[]>({
    queryKey: ['/api/transactions', 'credit'],
    queryFn: async () => {
      const response = await api.get('/api/transactions?type=credit&status=pending');
      return response.data?.data || [];
    }
  });

  // Filtrer les crédits selon le mode sélectionné
  const getPendingCredits = () => {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const nextWeek = addDays(today, 7);

    return credits.filter(credit => {
      if (credit.status !== 'pending') return false;
      
      const creditDate = parseISO(credit.date);
      
      switch (creditFilterMode) {
        case 'today':
          return isToday(creditDate);
        case 'week':
          return (isAfter(creditDate, today) || isToday(creditDate)) && isBefore(creditDate, nextWeek);
        case 'all':
          return true;
        default:
          return false;
      }
    });
  };

  const pendingCredits = getPendingCredits();

  // Navigation entre les crédits
  const scrollCredits = (direction: 'left' | 'right') => {
    if (direction === 'left') {
      setCurrentCreditIndex(prev => (prev > 0 ? prev - 1 : pendingCredits.length - 1));
    } else {
      setCurrentCreditIndex(prev => (prev < pendingCredits.length - 1 ? prev + 1 : 0));
    }
  };

  // Compter les crédits urgents (dus aujourd'hui ou en retard)
  const getUrgentCreditsCount = () => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    return credits.filter(credit => {
      if (credit.status !== 'pending') return false;
      if (!credit.date) return true;
      
      const dueDate = parseISO(credit.date);
      return !isAfter(dueDate, today);
    }).length;
  };

  // Remplacer le router.push par une navigation simple
  const handleNavigateToFinance = () => {
    window.location.href = "/finance";
  };

  return (
    <Card className="w-1/2 h-[140px] bg-gradient-to-br from-background/90 to-background/50 backdrop-blur-sm border-t-2 border-t-primary/30 hover:border-primary/50 transition-all duration-300 hover:shadow-lg overflow-hidden relative">
      <div className="absolute inset-0 w-[5px] h-full bg-gradient-to-b from-orange-500/70 to-orange-500/30" />
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-2 pl-4 relative z-10">
        <div className="flex items-center gap-2">
          <div className="p-1 rounded-full bg-background/80 shadow-md backdrop-blur-sm border border-border/20 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3 text-orange-500">
            <CreditCard className="h-4 w-4" />
          </div>
          <CardTitle className="text-sm font-medium flex items-center gap-1">
            <span>Crédits à payer{creditFilterMode === 'today' ? " (aujourd'hui)" : creditFilterMode === 'week' ? " (7j)" : ""}</span>
            {pendingCredits.length > 0 && (
              <div className="flex items-center gap-1 ml-2">
                <div className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">
                  {pendingCredits.length} en attente
                </div>
                {getUrgentCreditsCount() > 0 && (
                  <div className="text-[10px] px-1.5 py-0.5 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400">
                    {getUrgentCreditsCount()} urgent{getUrgentCreditsCount() > 1 ? 's' : ''}
                  </div>
                )}
              </div>
            )}
          </CardTitle>
        </div>
        <div className="flex items-center gap-1">
          <Button 
            variant="ghost" 
            size="icon"
            className="h-7 w-7 hover:bg-accent/50 rounded-full transition-all duration-300 hover:scale-110"
            onClick={() => scrollCredits('left')}
            disabled={pendingCredits.length <= 1}
          >
            <ChevronLeft className="h-3 w-3" />
          </Button>
          <Button 
            variant="ghost" 
            size="icon"
            className="h-7 w-7 hover:bg-accent/50 rounded-full transition-all duration-300 hover:scale-110"
            onClick={() => scrollCredits('right')}
            disabled={pendingCredits.length <= 1}
          >
            <ChevronRight className="h-3 w-3" />
          </Button>
          <Button 
            variant="ghost" 
            size="sm" 
            className={`h-7 text-xs ${creditFilterMode === 'today' ? 'text-red-500' : 'text-muted-foreground'}`}
            onClick={() => {
              setCreditFilterMode(creditFilterMode === 'today' ? 'week' : 'today');
              setCurrentCreditIndex(0);
            }}
            title={creditFilterMode === 'today' ? "Afficher les crédits des 7 prochains jours" : "Afficher uniquement les crédits d'aujourd'hui"}
          >
            Auj.
          </Button>
          <Button 
            variant="ghost" 
            size="sm" 
            className={`h-7 text-xs ${creditFilterMode === 'week' ? 'text-blue-500' : 'text-muted-foreground'}`}
            onClick={() => {
              setCreditFilterMode(creditFilterMode === 'week' ? 'all' : 'week');
              setCurrentCreditIndex(0);
            }}
            title={creditFilterMode === 'week' ? "Afficher tous les crédits" : "Afficher les crédits des 7 prochains jours"}
          >
            7j
          </Button>
          <Button 
            variant="ghost" 
            size="sm" 
            className={`h-7 text-xs ${creditFilterMode === 'all' ? 'text-green-500' : 'text-muted-foreground'}`}
            onClick={() => {
              setCreditFilterMode(creditFilterMode === 'all' ? 'week' : 'all');
              setCurrentCreditIndex(0);
            }}
            title={creditFilterMode === 'all' ? "Afficher les crédits des 7 prochains jours" : "Afficher tous les crédits"}
          >
            Tous
          </Button>
          <Button 
            variant="ghost" 
            size="sm" 
            className="h-7 text-xs text-muted-foreground hover:text-foreground"
            onClick={handleNavigateToFinance}
          >
            Voir tout
          </Button>
        </div>
      </CardHeader>
      <CardContent className="p-2 pt-0 pl-4 relative z-10">
        <div className="space-y-2">
          {pendingCredits.length > 0 ? (
            <AnimatePresence mode="wait">
              <motion.div
                key={currentCreditIndex}
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                transition={{ duration: 0.3 }}
                className="relative"
              >
                <div className="flex items-start justify-between w-full">
                  <div className="flex items-start gap-2 max-w-[60%]">
                    <div className="p-1 rounded-full bg-background/80 shadow-sm border border-border/20 mt-0.5">
                      <CreditCard className="h-3 w-3 text-orange-500" />
                    </div>
                    <div className="space-y-0.5">
                      <p className="text-xs font-medium truncate">
                        {pendingCredits[currentCreditIndex].property?.name || "Propriété non spécifiée"}
                      </p>
                      <p className="text-[10px] text-muted-foreground truncate">
                        {pendingCredits[currentCreditIndex].description || "Crédit sans description"}
                      </p>
                      <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <Clock className="h-3 w-3 flex-shrink-0" />
                        <span className="truncate">
                          {isToday(parseISO(pendingCredits[currentCreditIndex].date)) ? 
                            `Aujourd'hui à payer` : 
                            `À payer le ${format(parseISO(pendingCredits[currentCreditIndex].date), "d MMMM yyyy", { locale: fr })}`}
                        </span>
                      </p>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-1 max-w-[40%]">
                    <div className="flex items-center gap-2">
                      <div className="text-[10px] px-1.5 py-0.5 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 truncate">
                        {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(pendingCredits[currentCreditIndex].amount)}
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-1">
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-7 w-7 hover:bg-green-100 dark:hover:bg-green-900/20 rounded-full" 
                        title="Marquer comme payé"
                        onClick={() => setCreditToUpdate({ id: pendingCredits[currentCreditIndex].id, status: 'completed' })}
                      >
                        <CheckCircle className="h-4 w-4 text-green-500" />
                      </Button>
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-7 w-7 hover:bg-red-100 dark:hover:bg-red-900/20 rounded-full" 
                        title="Annuler le crédit"
                        onClick={() => setCreditToUpdate({ id: pendingCredits[currentCreditIndex].id, status: 'cancelled' })}
                      >
                        <XCircle className="h-4 w-4 text-red-500" />
                      </Button>
                    </div>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
          ) : (
            <div className="flex items-center justify-center h-12">
              <p className="text-xs text-muted-foreground">Aucun crédit en attente</p>
            </div>
          )}
        </div>
      </CardContent>
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
    </Card>
  );
} 